-- Ensure profiles RLS allows users to manage their own profile
...
-- Fix any NULL tokens in auth.users that break GoTrue
UPDATE auth.users
SET
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM auth.users WHERE email = 'joaozinhosantoss@icloud.com') THEN
    ...
    -- Manual profile seed fallback to ensure it exists
