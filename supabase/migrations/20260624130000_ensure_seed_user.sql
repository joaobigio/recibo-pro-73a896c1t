...
DECLARE
  new_user_id uuid;
BEGIN
  -- Seed user joaozinhosantoss@icloud.com (idempotent: skip if email already exists)
  IF NOT EXISTS (SELECT 1 FROM auth.users WHERE email = 'joaozinhosantoss@icloud.com') THEN
    ...
    -- Insert corresponding profile record referencing auth user id
    INSERT INTO public.profiles (id, email, name, is_admin)
    VALUES (new_user_id, 'joaozinhosantoss@icloud.com', 'Joãozinho Santos', true)
    ON CONFLICT (id) DO UPDATE SET is_admin = true;
  ELSE
    -- If exists, ensure tokens are not null and verify admin status
    UPDATE auth.users
    SET 
    ...
    -- Ensure the user is an admin in profiles
